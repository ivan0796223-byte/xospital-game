p = next((x for x in operation if x["id"] == id), None)
    if p:
        operation.remove(p)
...
@app.route("/lab")
def lab():
    return render_template("lab.html", result=random.choice(["🧪 Чисто", "🦠 Вирус", "⚠ Риск"]))
...
@app.route("/call/<car>")
def call(car):
    p = get_patient(random.randint(1, len(patients)))
    if p:
        rooms.append(p)
